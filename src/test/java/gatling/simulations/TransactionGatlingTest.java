...
public class TransactionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all transactions").get("/api/transactions").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created transaction").get("${new_transaction_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created transaction").delete("${new_transaction_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
