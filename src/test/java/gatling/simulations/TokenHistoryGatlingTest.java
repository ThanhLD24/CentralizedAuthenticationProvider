            ...
            exec(http("Get all tokenHistories").get("/api/token-histories").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .exec(
                    http("Create new tokenHistory")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_tokenHistory_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created tokenHistory").get("${new_tokenHistory_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created tokenHistory").delete("${new_tokenHistory_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TokenHistory entity").exec(scn);
